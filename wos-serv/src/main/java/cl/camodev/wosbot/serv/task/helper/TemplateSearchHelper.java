...
public class TemplateSearchHelper {
    ...
    /**
     * Optional preemption check hook, called before each inter-retry sleep.
     * Set via {@link #setPreemptionCheck(Runnable)} from the owning task.
     * Defaults to a no-op so the helper is safe to use without a task context.
     */
    private Runnable preemptionCheck = () -> {
    };
    ...
    /**
     * Sets the preemption check hook that will be invoked before each
     * inter-retry sleep. Pass {@code task::checkPreemption} from the owning
     * {@link cl.camodev.wosbot.serv.task.DelayedTask} so that a triggered
     * preemption is detected promptly even inside long retry loops.
     *
     * @param check A {@link Runnable} that throws a preemption exception when
     *              the task has been preempted; must not be {@code null}.
     */
    public void setPreemptionCheck(Runnable check) {
        this.preemptionCheck = Objects.requireNonNull(check, "preemptionCheck");
    }
     ...
     * Invokes the preemption check hook before sleeping so that a triggered
     * preemption is detected without waiting for the next tap/swipe/sleep in
     * the task layer.
    ...
    private void sleep(long milliseconds) {
        preemptionCheck.run(); // honour any pending preemption before sleeping
        try {
