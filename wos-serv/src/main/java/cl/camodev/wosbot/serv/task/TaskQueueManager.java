...
public class TaskQueueManager implements PreemptionListener {
        ...
        @Override
        public void onPreemption(DTOProfiles profile, PreemptionRule rule) {
                TaskQueue queue = taskQueues.get(profile.getId());
                if (queue != null) {
                        try {
                                queue.preemptCurrentTask(rule);
                        } catch (Exception e) {
                                logger.error("Preemption handling error [{}]: {}", profile.getName(), e.getMessage());
                        }
                }
        }

        public void startQueues() {
                // Initialize GlobalMonitorService by registering this manager as a listener
                GlobalMonitorService.getInstance().registerListener(this);

                // Register default global rules
                GlobalMonitorService.getInstance()
                                .registerRule(new cl.camodev.wosbot.serv.task.rules.BearTrapPreemptionRule());
                GlobalMonitorService.getInstance()
                                .registerRule(new cl.camodev.wosbot.serv.task.rules.GatherDeployPreemptionRule());
                GlobalMonitorService.getInstance()
                                .registerRule(new cl.camodev.wosbot.serv.task.rules.ManualRallyJoinPreemptionRule());
                GlobalMonitorService.getInstance()
                                .registerInjectionRule(
                                                new cl.camodev.wosbot.serv.task.rules.HelpAllianceInjectionRule());
                GlobalMonitorService.getInstance()
                                .registerInjectionRule(
                                                new cl.camodev.wosbot.serv.task.rules.FurnaceUpgradeInjectionRule());
                ...
                taskQueues.entrySet().stream()
                                ...
                                .forEach(entry -> {
                                        ...
                                        // Start global monitoring for this profile
                                        GlobalMonitorService.getInstance().startMonitoring(profile,
                                                        taskEnum -> queue.isExecutingTask(taskEnum)
                                                                        || queue.isTaskScheduledSoon(taskEnum, 10));
