...
public abstract class Emulator {
	...
	private boolean connectToDeviceBySerial(String serial) {
		try {
			// Check interrupt flag BEFORE starting a process
			if (Thread.currentThread().isInterrupted()) {
				logger.info("Thread interrupted before connecting to device: {}", serial);
				return false;
			}
			...
			logger.info("Connection to device interrupted (queue stopping): {}", serial);
	...
	protected <T> T withRetries(String emulatorNumber, Function<IDevice, T> action, String actionName) {
		...
		for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
			// Fast exit if thread was interrupted (queue stopping)
			if (Thread.currentThread().isInterrupted()) {
				logger.info("Thread interrupted, aborting {} on {} (Phase 1, attempt {})", actionName, emulatorNumber, attempt);
				throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber);
			}
			try {
				...
				logger.info("Thread interrupted during {} on {} (Phase 1, attempt {})", actionName, emulatorNumber, attempt);
				Thread.currentThread().interrupt();
				throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber, ie);
			} catch (Exception e) {
		...
		// Fast exit if thread was interrupted (queue stopping)
		if (Thread.currentThread().isInterrupted()) {
			logger.info("Thread interrupted, aborting {} on {} before Phase 2 (emulator restart)", actionName, emulatorNumber);
			throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber);
		}
		logger.warn(
		...
		try {
			...
			logger.info("Thread interrupted during emulator restart for {} on {}", actionName, emulatorNumber);
			Thread.currentThread().interrupt();
			throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber, ie);
		} catch (Exception e) {
		...
		for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
			// Fast exit if thread was interrupted (queue stopping)
			if (Thread.currentThread().isInterrupted()) {
				logger.info("Thread interrupted, aborting {} on {} (Phase 3, attempt {})", actionName, emulatorNumber, attempt);
				throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber);
			}
			try {
				IDevice device = findDevice(emulatorNumber);
				...
				logger.info("Thread interrupted during {} on {} (Phase 3, attempt {})", actionName, emulatorNumber, attempt);
				Thread.currentThread().interrupt();
				throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber, ie);
			} catch (Exception e) {
				...
				if (attempt >= MAX_RETRIES - 2) {
					...
					try {
						...
						Thread.currentThread().interrupt();
						throw new ADBConnectionException("Interrupted during " + actionName + " on " + emulatorNumber, iee);
					}
				} else {
					try {
						Thread.sleep(RETRY_DELAY_MS / 2);
    ...
    private DTORawImage captureScreenshotInternal(String emulatorNumber)  {
        ...
        try {
            ...
            // Pre-allocate ~3.7 MB to avoid repeated internal array resizing
            ByteArrayOutputStream imageData = new ByteArrayOutputStream(720 * 1280 * 4 + 64);
	...
	// ======================================================================
	// Public accessors for ScrcpyStreamCapture integration
	// ======================================================================

	/**
	 * Returns the ADB executable path resolved from the project's lib directory.
	 * Used by {@link ScrcpyStreamCapture} to push the scrcpy-server and set up
	 * port forwarding.
	 *
	 * @return Absolute path to adb.exe
	 */
	public String getAdbPath() {
		return getProjectAdbPath();
	}

	/**
	 * Returns the ADB device serial for the given emulator number.
	 * Used by {@link ScrcpyStreamCapture} to target the correct device.
	 *
	 * @param emulatorNumber Emulator identifier
	 * @return Device serial string (e.g. "127.0.0.1:16416")
	 */
	public String getPublicDeviceSerial(String emulatorNumber) {
		return getDeviceSerial(emulatorNumber);
	}
