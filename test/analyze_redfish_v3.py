"""
Redfish velocity analysis v3.
Only tracks RIGHT-FACING fish (no template flip).
This gives clean unidirectional tracks with no bounce complications.
Left-facing fish have the same speed, just mirrored on X.
"""
...
TEMPLATE_PATH = r"c:\Users\parad\Desktop\New folder\wosbot\wos-serv\src\main\resources\templates\fishingtoornament\redfishRight.png"
OUTPUT_CSV = r"c:\Users\parad\Desktop\New folder\wosbot\redfish_tracking_v3.csv"
ANNOTATED_DIR = r"c:\Users\parad\Desktop\New folder\wosbot\analysis_frames\annotated_redfish_v3"
THRESHOLD = 0.75
SCREEN_W = 720
UI_TOP_Y = 170
PLAY_BOT_Y = 1200
...
def detect_fish_right_only(frame, template, threshold):
    """Detect only RIGHT-facing fish. No flip."""
    ...
    return nms(detections, merge_dist=30)


def nms(detections, merge_dist=30):
    if not detections:
        return []
    ...
    for frame_idx, dets in all_detections:
        ...
        if frame_idx in track_lookup:
            for tid, cx, cy in track_lookup[frame_idx]:
                ...
                cv2.putText(frame, f"R{tid}", (cx - hw, cy - hh - 5),
...
def main():
    print("Loading redfish template (RIGHT-FACING ONLY)...")
    ...
    # Show multi-detection frames
    print("\n" + "=" * 80)
    print("FRAMES WITH >1 RIGHT-FACING REDFISH")
    print("=" * 80)
    for fidx, dets in all_detections:
        if len(dets) > 1:
            print(f"  Frame {fidx}: {len(dets)} at {[(d[0], d[1]) for d in dets]}")
    ...
    for t in good_tracks:
        ...
        if r2_y > 0.95 and r2_x > 0.95 and len(t.positions) >= 8:
            all_vx.append(vx)
            all_vy.append(vy)
    ...
    if all_vx:
        ...
        print("\n" + "=" * 80)
        print("╔══════════════════════════════════════════════════════════════╗")
        print("║     REDFISH CONSTANT VELOCITY — RIGHT-FACING ONLY          ║")
        print("╠══════════════════════════════════════════════════════════════╣")
        ...
        print("No high-confidence tracks (R²>0.95 on both axes).")
        print("Showing all tracks with >=5 pts:")
        for t in good_tracks:
            result = linear_regression_velocity(t)
            if result:
                vx, vy, r2_x, r2_y = result
